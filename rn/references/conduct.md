...
1. With `Feedback`: have what it revised evaluated, and decide.
2. Take the first task in `steering.md` not marked `[x]`:
   - **A task**: start a fresh agent with `Agent`, giving it the paths of
     `${CLAUDE_PLUGIN_ROOT}/references/implement.md` and `steering.md`, the task's id, and, on a
     retry, the More that decided it. It returns its commits and what it found to be the user's. Have
     them evaluated, and decide.
   - **A sign-off**: at a Design sign-off whose choices are not yet set out, set them out where the
     `design` field says, each with what it costs and gives, and your recommendation. Have what the
...
1. Decide by the Goal, from each More and from what the implementer returned. Nothing left between the
   work and its Purpose: mark the task `[x]`, or stop for the user at the sign-off, and with
   `Feedback`, reply on each pull request thread it came from with what changed and the commit, in
   the comment's language, leaving resolving it to the user, and set it to none. The implementer's or
   yours to fix: the task is retried by the next turn; the plan or the choices you revise and have
   evaluated again, past the Plan sign-off by adding tasks before the sign-off it stops at. The
   user's, taste, scope, cost against benefit, or a way that keeps falling short: it goes to them, at
   the sign-off you are stopping at, or at a Design sign-off that takes the place of every task not
   yet `[x]`.
