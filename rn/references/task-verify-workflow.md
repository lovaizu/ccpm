...
3. **Advance.** Begin the next unchecked task immediately at Phase: Execute (`task-execute-workflow.md`).
   If all tasks are done, run the **evaluation gate**: propose running the `steering.md` Acceptance
   criteria and get the user's sign-off on the result, taken via `/rn:ty` (approve) or `/rn:gm` (revise) —
