...
`on` and `up` read this file when they reach task execution. Run one task at a time.
...
- **Commit that check-off with the single completion marker** — message `{type}: complete task #{id} —
  ...
  substring. That exact substring is what `/rn:up` matches against `git log` when it reconciles tasks —
