...
by `task.md` (kinds Design, Deliverable, Session).
...
1. **Caller — give the judgment to someone who did not build the thing, so the verdict is not
   ...
   - a verdict file — a path outside the repository where it writes the verdict it returns, so
     the caller can pass it on without retyping a word;
...
4. **Caller — put every round's verdict, as returned, where the user reviews, so it is read in
   rendered form and stays addressable.** A line without evidence is not a verdict: send it back
   first. Then post the verdict file itself — `gh pr comment --body-file` on the session PR — read
   the posted comment back against the file, and remove the file. With no PR, print the file in
   full in the conversation instead.
