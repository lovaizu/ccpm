...
6. **Retire shipped decisions, collapse shipped tasks, then clean up State.**
   ...
   - In the `Tasks` section, for each shipped task — box checked in steering.md AND `complete task #{id}` marker in `git log` (the same trigger, per Step 4) — find the short SHA of its `complete task #{id}` commit and replace the whole task block (the `### #N: …` heading and everything under it, up to the next `### ` task heading) with the single line `### #N: <task name> — SHIPPED (#N in <sha>)`. Drop the body. Leave an in-progress or unshipped task fully intact.
