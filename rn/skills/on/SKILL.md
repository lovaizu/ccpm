...
3. **Create steering.md.** Read `${CLAUDE_PLUGIN_ROOT}/references/steering-template.md` and follow its per-section guidance. Fill `Goal`, `Acceptance criteria`, `Assumptions`, and `Rules`. Leave `Tasks`, `Decisions`, and `State` as their placeholders for now.
