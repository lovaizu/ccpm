...
3. **Start the session's branch.** Uncommitted changes in the tree are the user's → ask whether to
   set them aside first, so they do not land in the session. Create the branch from the default
   branch.
4. **Write the plan** from the template in `${CLAUDE_PLUGIN_ROOT}/references/steering.md`, reading
...
5. **Put it on record.** Commit `chore: start session — {slug}`, push, and open a draft PR whose
   body is
   ...
   the conversation.
6. **Run the session to its first decision** as in Running the session to its next decision in
