...
- [x] C：`04_proposal.md` を実デッキ17ページの正確な対応表に書き換え。さらに **骨格が欠く必須章**
      （なぜ私たちか/実績・チーム＝⚠️必須・見守り・連絡先）を明示し、既存スライド複製で足す指示を追加。
      .md(16章)/.html(17ページ)/SKILL⚠️ の三者ドリフトを解消（実物 grep で欠落確認：なぜ私たち=0・チーム=0・連絡=0）
...
- **Date**: 2026-06-23
- **Last completed**: **task #8**（シミュレーション評価由来の仕上げ4点）を SKILL.md・`04_proposal.md`・CHANGELOG に反映。
  A=内部見積前提インテイク節／B=フェーズ4 export 前の `{{ }}`・`例` 残留 grep ゲート／C=`04_proposal.md` を実17ページ
  対応表に書き換え＋骨格欠落の必須章（なぜ私たちか・見守り・連絡先）明示／D=フェーズ1クロール代替⚠️。
  `validate hposal --strict`・`validate . --strict` ✔。実物 grep で骨格の必須章欠落（なぜ私たち=0/チーム=0/連絡=0）を確認済み。
