...
- [ ] Phase 1：標準確認に⑥（301/SEO移行維持＝SC移転をスコープに含めるか）を追加（#1）／移行元が
      サブドメイン型（独自ドメイン非保有）なら301可否・SEO引き継ぎ制約を確認する⚠️（#2）
- [ ] Phase 2：toC（個人向け取引・予約・決済）があれば法務ページ群（特商法・プライバシー・キャンセル・
...
- [ ] Phase 3：完了条件に「明細列の作業見積合計＝役割別集計小計を機械的に突合（再加算で一致確認）」
      ゲートを追加（#9・実証済）／小規模案件は積み上げ後に省略/段階化できる行を★で握る⚠️（#5）
- [ ] 横断：トレース節に「現行ページ数（母数＝01）と新サイトのページ数は別。母数を新サイト規模として
...
- [ ] self-check（追記12点の有無を grep/目視で OK/NG 判定し `.rn/hposal-plugin/checks/9.md` に記録）
...
### #10: 提案書テンプレを3層構造＋基盤比較に改修する（中核）

**Purpose**: dogfood で判明したテンプレの歪み（複数サイト統合前提・総額1点前提・実現手段の比較層が無い）を
直す。`04_proposal.html`／`04_proposal.md` を「要件→オリジナルの提案→実現手段の提案」の3層に対応させ、
実現手段に本物の分岐がある時は比較を“層”として2列で通せる骨格を持たせる。見本＝private
`ikuko-hp/04_proposal.html` を PII 除去して一般化。CSS（1–376行）はバイト不変を保つ。`dogfood-notes.md` の
反映方針 (ii) に対応。
...
- [ ] `04_proposal.md` 章立てに「実現手段に施主が選ぶ本物の分岐がある時は『実現手段の提案』を独立した層と
      し、その層以降は採用候補を2列（N列）比較で通す。要件・共通提案は1列」を明示（#21）
- [ ] SKILL.md フェーズ4の章立て指示に同趣旨を1行（#21 の SKILL 側）／フェーズ2の #4（基盤未定なら2–3案を
      同じ軸＝初期・ランニング・更新主体・拡張性・保守 で比較し★で選ぶ）を見積モデルに足す
- [ ] `04_proposal.html` に骨格追加：基盤比較1スライド（ニュートラル・推奨マークなし）／2層=共通提案(1列)・
      3層=実現手段比較(2列で作り方・運用保守・見積を並走)／見積スライドに複数額フラット併記レイアウト（#19・#21）
- [ ] #10 単一サイト移行なら現状列を1本に畳む注記／#11 サービス紹介＋CTA画面型を追加し検索型は中〜大規模向けと
      位置づけ／#12 見積箱にレンジ表示パターン／#13 密表は内容5〜6行＋リード2行が目安の注記 — をテンプレに
- [ ] 既存クラス流用で CSS 1–376行はバイト不変（diff確認）・ヘッドレスChromeで全ページ16:9書き出し＋
      新スライド目視・export ゲート（`{{`/`例` 残留0）clean
- [ ] CHANGELOG `[Unreleased]` に追記し `validate hposal --strict`・`validate . --strict` 両方
- [ ] self-check（CSS diff・grep・PDF・代表スライド目視を OK/NG 判定し `.rn/hposal-plugin/checks/10.md` に記録）
- [ ] QA engineer review（subagent）
- [ ] user review

**Completion criteria**:

- `04_proposal.md` と SKILL.md フェーズ4が「実現手段の提案」層（比較で通す）を明示し、フェーズ2に #4 基盤比較が入っている。
- `04_proposal.html` が基盤比較スライド＋2列比較レイアウト＋複数額フラット併記の見積枠を骨格として持ち、
  #10–13 のテンプレ注記/型が入っている。CSS（1–376行）はバイト不変。
- ヘッドレスChromeで 16:9 全ページ書き出しが破綻なく、export ゲートが clean。
- CHANGELOG `[Unreleased]` に該当行があり、`validate --strict`（plugin/marketplace 両方）が通る。
