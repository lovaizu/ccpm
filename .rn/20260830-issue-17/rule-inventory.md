...
| 9 | "**Verification expert** (per medium: test / fact-check / dry-run)" | :23-24 | unseen | 35 of 40 past check files carry no Verification section, but that figure spans sessions the rule did not govern — it entered on 2026-07-01 (`b21620a`). Scoped to the sessions it governs, the breach is real and specific: `20260705-improve-design-template` wrote a Craft step into all 6 of its build tasks and a Verification step into none, and none of its 6 check files carries a Verification section. **6 post-rule breaches, in one session.** That session is the second confirmed instance this session's Assumptions name — not the criteria failure in the Goal (rows 129, 116). |
...
| 127 | Steps include "Verification expert review (subagent, per the task's medium)" | :70 | unseen | The second confirmed instance named in this session's Assumptions, and the one place a governed breach is demonstrable: `20260705-improve-design-template` wrote a Craft step into 6 of 6 build tasks (`:93`, `:123`, `:150`, `:176`, `:202`, `:227`) and a Verification step into none. The rule was in the template, the omission is plain in the output, and nothing in `rn` reads the output against the template. |
