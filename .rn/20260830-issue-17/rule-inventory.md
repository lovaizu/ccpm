...
**Counts.** 249 normative statements. By file: 40 in `task-execute-workflow.md` (whose lines 4–135 are
byte-identical to `task-verify-workflow.md`'s — verified by `diff` — so rows 1–26 state the rules of
...
| Check-file format's `## Overall Verdict` block (`task-execute-workflow.md:127-134`) | 24 of 40 check files have no Overall Verdict at all, so no `Ready to check off` line | Nothing; every one of those tasks was still checked off. |
...
| `Rn version:` stamped at creation (`planning-workflow.md:33`) | absent from 6 of 8 `steering.md` files | Nothing — and the absence is silent by construction, since the version check is "a plain string comparison" (`migration-workflow.md:4`) with no left operand. |
...
Lines 4–135 of this file are byte-identical to lines 4–135 of `task-verify-workflow.md` (verified by
`diff`); rows 1–26 therefore state the rules of both files and are not repeated in the next table.
...
| 6 | "**QA expert** (every task) — subagent." | :18 | neither | The check file has a `## QA Expert Review` section, but nothing reads it: 24 of 40 past check files lack the whole Overall Verdict block and every one of those tasks was still checked off. |
...
| 26 | The check-file format block — five columns, `## QA Expert Review`, three expert sections, `## Overall Verdict` with five verdict lines and `Ready to check off` | :86-135 | neither | Nothing reads the file's shape. 24 of 40 past check files omit `## Overall Verdict` entirely. `migration-workflow.md:46-48` asserts that "`task-verify-workflow.md`'s Phase: Complete only checks off once Verify has cleared with `Ready to check off` reading Yes" — but Phase: Complete (`task-verify-workflow.md:208-209`) never mentions the field. **The mechanism that file names does not exist.** |
...
Lines 1–135 carry rows 1–26 above; only the file's own Phase: Verify and Phase: Complete are listed here.
...
| 63 | "Record the review verdicts into the check file." | :191-192 | visible | The check file is committed with the check-off (`:83-84`) and lands in the PR. Empirically weak: 24 of 40 past files have no verdict block and nothing objected. |
...
| 68 | "This is the one completion marker for the task: deliverable commits carry plain messages; only this check-off commit carries the `complete task #{id}` substring. Keep that exact substring regardless of the prefix." | :212-214 | visible | Same consumer as row 67 — and demonstrably fooled: three commits in this repo carry the substring in prose. The grep is also not session-scoped, so `complete task #1` matches 10 commits across eight sessions. |
...
| 97 | "Commit it: `chore: start session — {slug}`." | :42 | visible | Confirmed used consistently in this repo (10 such commits in history). Nothing reads the message, but its absence would leave the file uncommitted and therefore undiscoverable by `dn`/`up`'s `git log` search. |
...
| 232 | "**Sync tasks.** Cross-check `git log` against the unchecked tasks. A commit matches a task when its message contains `complete task #{id}`; check that task off" | `up/SKILL.md:26` | stops | A real grep against real commits — the single most consequential consumer in `rn`. It is also unscoped to the session: `complete task #1` matches 10 commits across eight sessions in this repo, and three further commits carry the substring in prose. The mechanism exists and is wrong. |
...
- **`up/SKILL.md:26`'s grep is not session-scoped and matches prose.** `complete task #1` matches 10
  commits across eight sessions in this repo, and three commits (`4daf6b2`, `893de07`, `de0b1ec`)
  carry the substring in a body without being markers. The mechanism exists and produces wrong answers.
...
(row 93, itself unenforced and absent from 5 of 8 sessions). The template's own contract —
...
comparison" (`migration-workflow.md:4`) that no file defines for a missing line — so an un-stamped
...
- **Rows 26 and 162** — the check file's `## Overall Verdict` block, and `migration-workflow.md:46-48`'s
  ...
  `task-verify-workflow.md:208-209` contains no such condition. 24 of 40 past check files have no
  Overall Verdict block at all and every one of those tasks was checked off. **This is a mechanism
...
`task-execute-workflow.md` lines 4–135 and `task-verify-workflow.md` lines 4–135 are byte-identical —
132 duplicated lines with **no rule anywhere requiring they stay in sync**, and no mechanism if there
