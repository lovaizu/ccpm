...
**`rn` ships one line of machinery, and it guards one property.** Of the 269 normative statements in
its 8 reference files and 5 skill files, 61 are enforced and 208 are not. The plugin contains no
...
The 208 unenforced rows are this document's product: the definite list task #2 works from, grouped
under "The unenforced set" by *why* nothing catches them, because the groups need different repairs.
The two largest groups hold 72 of the 208 between them. Thirty-five rows produce something a user
...
prompts that are never written to disk. A tenth group, U10, holds the 30 rows that name a real
...
Applying this to a fixed point moved 27 rows out of the enforced set (30 sit in the group today, three of them never enforced to begin with); they are listed as **U10**,
...
with three typographic conventions applied throughout. A fragment broken across source lines is
rejoined. A command or identifier the source leaves bare is set in backticks (a `|` inside one is
escaped as `\|` so the table renders). And `…` marks material dropped from *inside* a quotation —
never from its end: a quotation that stops before the source sentence does simply stops, in the
per-file tables and in the group indexes alike, where every Rule cell is trimmed to a fixed budget
and closes with `…`. Capitalization and punctuation are the source's; where a quotation begins
mid-sentence its first word keeps the case the source gives it.
...
Several rules that read as enforced turn out to have been broken with nothing noticing. Two cautions
travel with every such count. **The rule-era is established before the breaches are counted** — a
rule that entered on 2026-07-01 (`b21620a`, `2ddcafa`: the Craft, Verification and Evaluation
sign-off rules) cannot have been breached by the six sessions that predate it, so those figures are
reported as coverage, never as breaches. **And only the artifact record is evidence.** The
repository retains no conversation transcripts, so what a user did or did not remark on is not
observable from it; where a claim of "nothing noticed" is made here it means a specific absence on
disk — no commit, no check-file entry, no escalation record — and it says which.

**Granularity.** One row per statement a reader could breach on its own. A template block is
expanded the same way as prose: each field or section that prescribes content gets a row, because
that content is what a later step addresses by name — which is why `steering-template.md`'s block
yields rows 115–134 and 255–257, and `design-template.md`'s yields rows 173 and 266–269, one per h2
section with its h3 questions. The instructions *about* a block (copy it verbatim; answer every
question) stay one row each (106, 169, 170), and a line that restates a rule already inventoried is
cited on that rule's row rather than given its own (`steering-template.md:32`, `:33` on rows 113 and
114).

**Counts.** 269 normative statements. By file: 43 in `task-execute-workflow.md` (whose shared span
with `task-verify-workflow.md` is counted once — see that file's table below), 32 more in
`task-verify-workflow.md`, 39 in `planning-workflow.md`, 42 in `steering-template.md`, 13 in
`status-display.md`, 11 in `migration-workflow.md`, 20 in `design-template.md`, 27 in
`pr-feedback-workflow.md`, and 4 / 14 / 11 / 7 / 6 in `on` / `dn` / `up` / `ty` / `gm`. Row ids are
stable identifiers assigned in the order the rules were inventoried; rows 250–269 sit at the end of
...
are recognized on breach**, **208 are unseen**.
...
| 3 | "Write check files under `{steering_dir}/checks/`." | :7-8 | unseen | No step reads back the path, and no past session has broken this rule — `20260623-rename-cmds-rdy-brb-bak` and `20260624-rename-cmds-on-dn-up` wrote `checks/task-1.md`, which is under `checks/` as required. What they broke is the `checks/{task-id}.md` filename pattern (rows 4, 33, 162), and nothing reads that back either. |
...
| 8 | "**Craft expert** (per medium: coding / writing / visual)" | :21-22 | unseen | Absent from every session before `20260625-rn-lean`; 29 of 40 past check files carry no Craft section. That figure is not a breach count: the rule entered on 2026-07-01 (`b21620a`, `2ddcafa`), and 20 of the 29 belong to sessions that predate it. Of the two sessions the rule governs, `20260705-improve-design-template` is 6 of 6 compliant and this one is in flight. **Post-rule Craft breaches: 0.** What the rule lacks is a mechanism, not a body count. |
| 9 | "**Verification expert** (per medium: test / fact-check / dry-run)" | :23-24 | unseen | 35 of 40 past check files carry no Verification section, but that figure spans sessions the rule did not govern — it entered on 2026-07-01 (`b21620a`). Scoped to the sessions it governs the breach is real and specific, and row 127 records it: `20260705-improve-design-template` wrote a Craft step into all 6 of its build tasks and a Verification step into none. |
...
| 12 | "The user signs off at exactly **three** scheduled gates, never on any other task" | :32 | unseen | An extra gate is a message on the user's screen; a missing one is a task that closes without them. Fails at leg (3): the user has no copy of the three-gate rule, so an extra ask reads as an ordinary question and a missing gate reads as smooth progress. The Design gate is the one to watch, and the statistic often quoted against it does not hold — `planning-workflow.md:36` makes the separate task conditional, so most sessions owed none (row 93). |
...
| 14 | "**Design gate** — sign-off on the approach / key decisions before they are built on" | :35-37 | unseen | Realized either as a Design sign-off task (`planning-workflow.md:36`) or, when the design is settled at plan time or the session has no `design.md`, folded into the plan gate (`:36`, `:49` — rows 102, 261). Which branch was taken is recorded nowhere by rule, so nothing holds a session's task list up against `planning-workflow.md:36`. Fails at leg (3) for the same reason as row 12: a session with no Design gate at all and one that folded it in present identically to the user. |
...
| 36 | Element 6 — plain conventional message; "the message must **not** contain `complete task #`"; push; "**never force-push**" | :163-165 | recognized | Real downstream consumer: `up/SKILL.md:26` — "A commit matches a task when its message contains `complete task #{id}`; check that task off". A breach silently checks off the wrong task, and one commit in this repo's history proves it: `de0b1ec` ("so this is wip not 'complete task #3'") carries `#3`'s substring in a body without being a marker. Two more commits quote the rule with the literal `{id}` in place (`4daf6b2`, `893de07`) and match no task's grep — they are not breaches. Force-pushes surface as events on the PR timeline; nothing in `rn` checks. |
...
| 258 | "Per-task quality on every other task is caught by self-check + QA/expert review + the coordinator's independent review." | :46-47 | unseen | The second place `rn` documents an enforcement it never built (row 26 and `migration-workflow.md:47-48` are the first). All three named catchers are unenforced here: the self-check is written by the agent that built the artifact (rows 32, 33), the QA/expert reviews leave no trace that they ran (rows 6–9, 45–55), and the coordinator's independent review is rows 41–44, which record nothing. The sentence is the plugin's own claim that per-task quality is caught, and it is the claim this inventory contradicts. |
| 259 | Coordinator duties — "Decomposes the goal, picks which expert each piece needs, dispatches, reviews against the committed diff, triages findings, re-instructs, updates `steering.md`, talks with the user." | :12-14 | unseen | The role's own duty list, and every item on it is unenforced under its own row: dispatch (5, 10, 40), review against the diff (41–44), triage (56–62), `steering.md` updates (66 is the one exception, enforced). Nothing records which of the duties the coordinator performed on any given task. |
...
| 63 | "Record the review verdicts into the check file." | :191-192 | unseen | Fails at leg (1): the check file is committed onto the PR (`:83-84`), but no reader is directed to it and no step gates on what it says — row 26's record is the proof. It is also the reader step five rows of U10 rest on (23, 24, 32, 33, 50), so a single line recording that the coordinator opened the file would carry six rows, not one. |
...
| 68 | "This is the one completion marker for the task: deliverable commits carry plain messages; only this check-off commit carries the `complete task #{id}` substring. Keep that exact substring regardless of the prefix." | :212-214 | recognized | Same consumer as row 67, and demonstrably fooled — by prefix collision, not by prose. The substring test is a prefix test: `git log --all --grep='complete task #1' --oneline \| wc -l` returns 16, of which 7 are other tasks (`#1b`, `#10`, `#11`, `#12`, `#13`, `#14`, `#16`). Exactly one commit carries a real task's substring in a body without being a marker — `de0b1ec` ("so this is wip not 'complete task #3'"), which `git log --all --grep='complete task #3' --oneline` returns alongside the seven genuine `#3` markers. The other two commits often cited here, `4daf6b2` and `893de07`, contain the literal `{id}` and match no task's grep. |
...
| 71 | "If no unchecked tasks remain and no "Evaluation sign-off" task was ever encountered … that is a planning defect … escalate to the user immediately … do not close the session silently." | :219-222 | unseen | Fails at leg (3): the breach is a session closed silently instead of escalated, and a user receiving a close report holds no copy of this rule to miss the escalation against. It is one of two places `rn` checks another rule's output — `dn`'s step 8 re-reads step 6's (row 224) — and neither is checked itself. Untested besides — the rule and `planning-workflow.md:37` both entered on 2026-07-01 (`2ddcafa`), and both sessions since have placed the Evaluation sign-off task, so it has never had an omission to catch. |
...
| 86 | "Stamp the template's top `Rn version:` line with the currently installed plugin's version — read from … `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` `version` field." | :33 | unseen | Only 2 of the 8 sessions carry an `Rn version:` line — `20260705-improve-design-template` and this one; the other 6 have none. The rule entered on 2026-07-01 (`2ddcafa`), so five of the six predate it and one breach, `20260625-rn-lean`, is genuinely post-rule. The consumer (`on/SKILL.md:15` and the four siblings) does "a plain string comparison" (`migration-workflow.md:5`) that no file defines for a missing line, so an omission is silently a match either way. |
...
| 92 | "Define each task following the template's `Tasks` structure, inline `Completion criteria` rules, and `Task definition requirements` table in full." | :35 | unseen | The parent of the per-task rules: rows 122–132 (the `Tasks` structure and its inline `Completion criteria` rules) and rows 135–141 (the `Task definition requirements` table). Every one of them is unenforced, and the omission recorded at row 127 is an omission of this step — planning wrote the tasks and left one mandated Step out of all six. |
| 93 | "**Design sign-off task.** When the session has a `design.md` not settled at plan time, place a **"Design sign-off"** task … at the point where heavy build would otherwise start" | :36 | unseen | No rule checks that the task exists; the Evaluation sign-off's backstop (row 71) has no counterpart here, and that backstop is itself `unseen`. **The breach count is 0 of 8, not 7 of 8.** `planning-workflow.md:36` places the task only when the session has a `design.md` *not settled at plan time*, and only 3 of the 8 sessions have a `Design:` line at all (`grep -n '^Design:' .rn/*/steering.md`). Both past sessions that owed the question recorded the fold-in — `.rn/20260705-improve-design-template/steering.md:68-69` and `.rn/20260625-rn-lean/steering.md:78` — as does row 102 from Step 5's side. What is unenforced here is the judgment, not a demonstrated omission. |
...
| 95 | "**Self-check before persisting.** Before persisting (Step 5), confirm the last task in the list is "Evaluation sign-off" — if it is not, add it before persisting." | :38 | unseen | A self-check with no recorded output. `20260625-rn-lean` placed its Evaluation sign-off as `#15`, mid-list before `#6`–`#14` — the check either did not run or ran and passed on a list it should have failed, and nothing on disk distinguishes the two. |
...
| 260 | "Read by `on/SKILL.md` at the start of a session, before task #1 begins — task execution then continues via `task-execute-workflow.md` / `task-verify-workflow.md`." | :3-5 | unseen | The read-order rule for planning, and the class rows 1 and 250 belong to: nothing records which file was read or when, so a session that ran planning from memory and one that read the file leave the same `steering.md`. |
| 261 | "When the design is settled at plan time — or the session has no `design.md` at all (Step 3's "force no empty `design.md`" rule) — place no separate task; it folds into the plan-gate hand-off instead (Step 5)." | :36 | unseen | The other arm of row 93, and the branch 2 of the 8 sessions actually took (`.rn/20260705-improve-design-template/steering.md:68-69`, `.rn/20260625-rn-lean/steering.md:78`). Nothing reads a task list against it: a session that folded the gate in and one that simply forgot the task produce the same `steering.md`, which is what makes row 93's absence uncountable. Row 102 states the same branch from Step 5's side. |
| 262 | "**Persist and open a draft PR — the plan gate.** This is the first of the three scheduled user sign-offs (plan / design / evaluation)." | :40 | unseen | The step header that names the gate. The persist and PR halves are rows 96 and 99; what this line adds is the count — first of three — and that is row 12's rule, failing at leg (3) for row 12's reason: the user holds no copy of the three-gate scheme, so a session that runs two gates or four reads to them as an ordinary session. |
...
| 264 | "Allocate content by kind, so steering stays a lean forward contract:" | :13 | unseen | The parent of rows 109–111 and the statement of what the doc-division is *for*. Nothing classifies content by kind (row 109), so the property this line names — that steering stays lean — is measured by nothing; `.rn/20260625-rn-lean/steering.md` runs to 350 lines with per-task narration in its headings and nothing in the flow reads its length or its contents against this rule. |
...
| 112 | "A decision lands in a task, in `design.md`, or in a rule. Deliberation and history live in git + the PR — never in steering." | :19 | unseen | Same absent reader as row 109. Searched the eight past `steering.md` files for a place where deliberation would be flagged: none exists, and `.rn/20260625-rn-lean/steering.md`'s task headings carry per-task narration (`— DONE through QA`, on 15 of 16 tasks) that no check file or commit records as a finding. |
| 113 | "`Rn version:` … is written once, at creation, from the installed plugin's version — never user-edited afterward" | :21-24 | unseen | Row 86: the line is usually absent, and the comparison that would consume it treats absence as no-mismatch. The template block states the same rule again at `:32` ("stamped at creation, not user-edited"), which is the rule restated, not a check. |
| 114 | "The `Design:` line below it points to the session's `design.md`. A session with no design omits this line entirely" | :26-27 | unseen | Row 84's consumer (`migration-workflow.md:28-29`), and row 84's cap. The template block carries the same instruction at `:33` ("omit this whole line if the session has no design"), which row 90 states as an obligation. |
...
| 265 | Task heading shape — "`### #N: <task name>`" | :57 | unseen | Load-bearing and unread. The `#N` it carries is what `up/SKILL.md:26`'s marker grep checks off against (row 140) and what `status-display.md:39-50` collapses into ranges "with a short label per task" — the label is this heading's text. But nothing parses `steering.md`'s headings: `20260625-rn-lean` appended `— DONE through QA` to 15 of its 16 headings and `20260830-issue-17` writes a sentence-long name, and both render and check off exactly as a bare `### #N: <name>` would. |
...
| 126 | Steps include "Craft expert review (subagent, per the task's medium)" | :69 | unseen | Absent from all 5 sessions before `20260625-rn-lean`, all of which predate the rule (`b21620a`, 2026-07-01). Post-rule, `20260705-improve-design-template` writes the step into 6 of its 6 build tasks. No breach to catch yet; nothing that would catch one. |
| 127 | Steps include "Verification expert review (subagent, per the task's medium)" | :70 | unseen | **The second confirmed instance this session's Assumptions name**, and the one place a governed breach is demonstrable: `20260705-improve-design-template` wrote a Craft step into 6 of 6 build tasks (`:93`, `:123`, `:150`, `:176`, `:202`, `:227`) and a Verification step into none, and none of its 6 check files carries the section. The rule was in the template, the omission is plain in the output, and nothing in `rn` reads the output against the template. Rows 9, 55, 92 and 139 are the same failure seen from their own files. |
...
| 139 | Criteria vs steps: criteria answer ① and ② with grounds, "not that an artifact was produced; actions, reviews, and gates go in Steps as `- [ ]` so their status stays trackable. The task-execution references' … Process selection section … is the source of *which* reviews apply — keep the two in sync" | :104 | unseen | "Keep the two in sync" names no mechanism, and the two are demonstrably out of sync: `task-execute-workflow.md:59-64` mandates Verification for every build task, this template's Steps list has carried it since `0.7.0`, and the next session wrote it into 0 of its 6 build tasks while writing Craft into all 6. Nothing compares the two documents, and nothing compares a written task to either. |
...
| 255 | `State` field: "**Date**: YYYY-MM-DD" | :88 | unseen | Read by `up/SKILL.md:24` along with the rest of `State`, which is row 231, `unseen` — and nothing parses the value, so a wrong or absent date is a wrong or absent string. Row 217 covers writing the section; this covers what the field must contain. |
| 256 | `State` field: "**Last completed**: #N description" | :89 | unseen | Row 255's reader. This is the field `up/SKILL.md:24` names first ("last completed task"), so a wrong id resumes from the wrong place — but the read is row 231 and records nothing, and no step re-derives the value from `git log` before using it (`up`'s own step 4 syncs the *task list*, not this field). |
| 257 | `State` field: "**Next**: #N description" | :90 | unseen | Row 255's reader, and the same failure. Together with rows 133 and 134 these three are the whole payload `up/SKILL.md:24` reads; all four fields are unenforced, and rows 133 and 134 are unenforced only because their readers are (U10). |
...
| 156 | "Coordinator only: no implementation expert, and no QA/Design/Craft/Verification review, spawns for this procedure itself" (`:8-9`), and "The coordinator reads each artifact, judges drift, and edits it directly." (`:12`) | :8-12 | unseen | Fails at leg (1): the reconciliation commit lands on the PR (`:56`), but no reader is assigned to it, and the property in question — that no expert was spawned — leaves no trace in a commit either way. The coordinator's own reading is stated at `:12` ("The coordinator reads each artifact, judges drift, and edits it directly") and records nothing. |
...
| 159 | Step 2: "Read the session's `steering.md` `Design:` line; if it is absent … skip this step entirely. If present, follow `design-template.md`'s own "Updating an existing design.md" procedure … do not re-derive reconciliation logic here." | :27-31 | unseen | Fails at leg (2): the `Design:` line is a real input to the branch, but the branch writes nothing, so there is no artifact in which a step taken and a step skipped differ. A read error on a dangling pointer (row 90) is the pointer's failure, not this step's; a step skipped outright errors on nothing. Four rows of U10 (84, 87, 90, 114) name this step as their reader, and it has never run (row 155). |
...
| 173 | Template block, section 1 — **Background & Goals**, and its four questions: "What is the goal?", "What goes wrong without this?", "What does reaching it require?", "What is out of scope?" | :40-48 | unseen | Same as row 170, and the header line is the file's own statement that its product has no runtime reader (row 166). |
| 266 | Template block, section 2 — **Assumptions & Constraints**: "What do we take as true?", "What binds the solution?" | :50-54 | unseen | Same as row 170. |
| 267 | Template block, section 3 — **Design overview**: "What is the core idea, and why does it solve the problem?", "What are the pieces, and what is each responsible for?", "How does work move?" | :56-62 | unseen | Same as row 170. |
| 268 | Template block, section 4 — **Detailed design**: "one 4.N subsection per mechanism/component the design introduces — repeat, do not cap at a fixed count", each asking "What does <mechanism> guarantee, and how is a breach caught?" | :64-68 | unseen | Same as rows 170 and 171. This is the question this inventory asks of every rule, asked of every mechanism in a design, and nothing checks that the answer is real — `rn/docs/design.md`'s own `4.N` answers are where this session's gap should have shown. |
| 269 | Template block, section 5 — **Alternatives considered**: "Why this shape, and not another?", "What did we trade away?" | :70-74 | unseen | Same as row 170. |
...
| 175 | "Follow this instead of the Steps above when `planning-workflow.md`'s location check (Step 2) points `Design:` at an existing `design.md`" | :100-102 | unseen | Row 168's branch, from the update procedure's side: nothing records which path was taken, and `migration-workflow.md:27-31` routes to this procedure without writing that it did (row 159). |
...
| 178 | "**The decision-plus-reasoning contract from fresh authoring still applies to whatever you touch.**" | :109-111 | unseen | Row 170's contract, carried into the update path. It has the same absent reader: a section revised without a decision or its reasoning reads as prose, and nothing opens `design.md` at runtime (`design-template.md:4`). |
...
| 263 | "**Execution subagent** — subagent (Agent tool, no conversation history)." | :17 | unseen | Word for word row 11's rule in the other file, and it fails identically: nothing records whether the Agent tool was used or whether history was passed, and a coordinator handling the thread itself posts the same reply. |
...
| 203 | "**Never resolve the thread.** Resolution is the author's act on GitHub" | :124 | recognized | Row 185's reader — the PR thread's reviewer, who sees a thread resolved by the assistant rather than by themselves, and whose resolution is what `:144-145` assigns to them. |
...
| 206 | "The loop **never** resolves a thread. Neither the coordinator nor the subagent calls `resolveReviewThread` … The loop treats GitHub's unresolved state as its **queue**." | :142-148 | recognized | State lives in GitHub rather than in the assistant's memory, so a thread the loop missed — dropped by a skipped page (row 190), filtered out wrongly (row 191), or never replied to (row 200) — is still unresolved on the next run and is re-collected. Quoted in full because the guarantee is real: it is the only rule in `rn` that repairs its own omission. |
| 207 | "One coordinator pass per item … is the whole of verification. No QA expert, no Design / Craft / Verification experts, no multi-round iteration cap." | :152-153 | unseen | Row 183's failure, restated at the end of the file: a scope statement about what does *not* run. Running the expert chain anyway would leave check files and review sections, but no step looks for their absence or their presence in a PR-feedback pass. |
...
| 219 | "The message must not contain `complete task #`." | `dn/SKILL.md:33` | recognized | Row 36's consumer. The nearest historical instance is `4daf6b2`, a suspend-time bookkeeping commit that quotes this very rule in its body — with the literal `{id}` in place, so it matches no task's grep and is a near miss rather than a breach. The one commit that does false-positive is `de0b1ec` (row 36). |
...
| 222 | "Never delete a file yourself." | `dn/SKILL.md:44` | unseen | A deleted untracked file leaves no trace anywhere — not in git, not in `git status`, not in `State → Notes`, which records only what step 8 still finds. Step 6's and step 8's records are both of paths that remain; a path that was removed is simply absent from both. |
...
| 225 | "**Report.** Open the report with the session-status block … then output the branch name. If the last push did not succeed, state that the commits are local-only and must be pushed. Name any user-deferred paths" | `dn/SKILL.md:56-59` | unseen | Fails at leg (2): the report is on the user's screen, but the flow does not send anyone to check its one checkable claim — that the commits are local-only — against the PR. Nothing in `rn` re-reads a sent message. |
...
| 235 | "**Check version.** … on a mismatch, run `…/references/migration-workflow.md` first — on a match, do nothing." | `up/SKILL.md:32` | unseen | Row 86's failure, at the resume end: `steering.md` usually carries no `Rn version:` line, the comparison is "a plain string comparison" (`migration-workflow.md:5`) that no file defines for a missing operand, and a resume that skips the check produces the same next commit as one that ran it. |
...
| 239 | "**Check version.**" | `ty/SKILL.md:13` | unseen | Row 86's failure, at the approval end. `/rn:ty` states the step in two words and names no operands; with most sessions carrying no `Rn version:` line there is nothing to compare, and nothing records the comparison either way. |
...
| 245 | "**Check version.**" | `gm/SKILL.md:13` | unseen | Row 86's failure, at the revise end — the same two-word step as row 239, with the same missing operand and the same absent record. |
...
| 247 | "**With feedback (`$ARGUMENTS` present).** Treat `$ARGUMENTS` as a revise verdict on the pending item … If there is no pending item, treat `$ARGUMENTS` as a direct instruction and act on it … do not stall on a missing target. Apply the revision … then report, opening the report with the session-status block … Do not enter the PR-feedback loop." | `gm/SKILL.md:17` | unseen | Fails at leg (1): the revision's commits are on the PR and the report is on the user's screen, but nobody is asked to judge whether they address the feedback. Contrast row 249's other arm, where an unaddressed PR thread stays unresolved and is re-collected; `$ARGUMENTS` feedback exists only in the conversation and leaves no queue behind. |
...
**208 rows** where a breach reaches nothing that would tell it from compliance. The ten groups
below are a partition — every `unseen` row appears in exactly one group, and
35+15+10+18+15+10+28+10+37+30 = 208 — and they are grouped by *why* nothing catches them, because the
...
citations travel with the ids here, the twenty rows appended out of numeric order (250–269) are
locatable in their files from this list alone.

### U1. The review chain's internals — 35 rows
...
| 259 | `task-execute-workflow.md:12-14` | Coordinator duties — "Decomposes the goal, picks which expert each piece needs, dispatches, reviews against the committed diff, triages findings …" |
...
| 263 | `pr-feedback-workflow.md:17` | "**Execution subagent** — subagent (Agent tool, no conversation history)." |
...
`20260705-improve-design-template` writes the step into none of its 6 build tasks and none of its 6
check files carries the section. That session is the only one the rule governed and its 6 tasks are
the only demonstrable breaches; the 35-of-40 figure spans five sessions that predate the rule and is
coverage, not a breach count. The session merged as PR #16 under the 1-approval ruleset, with its
own `#7: Evaluation sign-off` left with all three steps unchecked and no `complete task #7` commit —
so the sign-off it closed on is not recorded anywhere in the repository.
...
review (U2, row 127) is the second instance of the same mechanism, not the same failure. One asymmetry to fix
...
### U4. The whole of `design.md` — 18 rows
...
| 169 | `design-template.md:16-18` | "**Copy the template block below verbatim.** Keep every heading, numbering, and section order — the five h2 sections and the h3 questions under them …" |
...
| 173 | `design-template.md:32-75` | Template block, section 1 — **Background & Goals**, and its four questions: "What is the goal?", "What goes wrong without this?", "What does reaching …" |
| 266 | `design-template.md:50-54` | Template block, section 2 — **Assumptions & Constraints**: "What do we take as true?", "What binds the solution?" |
| 267 | `design-template.md:56-62` | Template block, section 3 — **Design overview**: "What is the core idea, and why does it solve the problem?", "What are the pieces, and what is each …" |
| 268 | `design-template.md:64-68` | Template block, section 4 — **Detailed design**: "one 4.N subsection per mechanism/component the design introduces — repeat, do not cap at a fixed …" |
| 269 | `design-template.md:70-74` | Template block, section 5 — **Alternatives considered**: "Why this shape, and not another?", "What did we trade away?" |
...
93, itself unenforced; only 3 of the 8 sessions have a `design.md` at all, and both past ones folded
the gate into the plan gate rather than skipping it). Rows 173 and 266–269 are the five h2 sections
of the template block, one row each, and rows 167 and 170 are the obligation to answer their h3
questions. The template's own contract — "What does <mechanism> guarantee, and how is a breach
caught?" (`design-template.md:68`, row 268) — is exactly the question this inventory asks, and
nothing checks that a `4.N` answers it truthfully.

### U5. Content allocation, doc division, and the migration judgments — 15 rows
...
| 264 | `steering-template.md:13` | "Allocate content by kind, so steering stays a lean forward contract:" |
...
The `Rn version:` stamp is present in only 2 of the 8 sessions, and its consumer is "a plain string
comparison" (`migration-workflow.md:5`) that no file defines for a missing line — so an un-stamped
session silently reads as up to date, forever. Five of the six omissions predate the rule
(`2ddcafa`, 2026-07-01); `20260625-rn-lean` is the one post-rule breach, and nothing in its record
marks it. `on/SKILL.md:15` documents that its own copy of the
...
### U7. Breaches with no possible trace — 28 rows
...
| 260 | `planning-workflow.md:3-5` | "Read by `on/SKILL.md` at the start of a session, before task #1 begins — task execution then continues via `task-execute-workflow.md` / …" |
...
| 258 | `task-execute-workflow.md:46-47` | "Per-task quality on every other task is caught by self-check + QA/expert review + the coordinator's independent review." |
...
| 261 | `planning-workflow.md:36` | "When the design is settled at plan time — or the session has no `design.md` at all (Step 3's "force no empty `design.md`" rule) — place no separate …" |
...
- **Rows 26, 162 and 258** — the three places `rn` asserts an enforcement it never built.
  `migration-workflow.md:47-48` claims Phase: Complete gates on `Ready to check off` reading Yes (see
  the enforced set, group D); `task-execute-workflow.md:46-47` says per-task quality "is caught by
  self-check + QA/expert review + the coordinator's independent review", and rows 6–9, 32–33 and
  41–45 say none of those three catches anything; row 26 records the cost of the first — 24 of 40
  check files omit the `Ready to check off` line and every one of those tasks was checked off.
...
### U9. Output on the user's screen with no auditor — 37 rows
...
| 262 | `planning-workflow.md:40` | "**Persist and open a draft PR — the plan gate.** This is the first of the three scheduled user sign-offs (plan / design / evaluation)." |
| 265 | `steering-template.md:57` | Task heading shape — "`### #N: <task name>`" |
...
| 156 | `migration-workflow.md:8-12` | "Coordinator only: no implementation expert, and no QA/Design/Craft/Verification review, spawns for this procedure itself" (`:8-9`), and "The …" |
...
### U10. Enforced only if a step nothing records actually runs — 30 rows
...
| 255 | `steering-template.md:88` | `State` field: "**Date**: YYYY-MM-DD" |
| 256 | `steering-template.md:89` | `State` field: "**Last completed**: #N description" |
| 257 | `steering-template.md:90` | `State` field: "**Next**: #N description" |
...
reading happened. Six reader steps carry all 30:
...
| `up`'s candidate ranking and `State` read — `up/SKILL.md:19`, `:24`; `dn/SKILL.md:15-16` | 214, 229, 231 | 106, 107, 133, 134, 217, 220, 221, 234, 255, 256, 257 |
...
would move most of the 30 back across the boundary at once, and none of them needs a new reader.
...
### Outside the 208: the rule that was never written
...
reviewer to be skipped. What the record shows is not proportionality but omission: planning stopped
writing the steps, and no artifact anywhere marks that as a defect.
...
| **3a. Design sign-off task** (conditional: a `design.md` not settled at plan time) | `:36` | 93, 254, 261 | That an unsettled design is approved before heavy build is spent on it. | Yes, and it is the one sign-off with no backstop. The condition is narrow, though: only 3 of the 8 sessions have a `design.md` at all, and both past ones settled it at plan time and recorded the fold-in. Nothing has yet been in a position to breach it. |
...
| Pre-persist self-check that the last task is Evaluation sign-off | `planning-workflow.md:38` | 95 | Would catch the omission at authoring time rather than at session end. | Yes in principle; unenforced in fact (row 95), and it covers only this one task. The Design sign-off, the four review steps and the criteria phrasing — the rules U2 and U3 hold — get no equivalent check at authoring time, which is where their breaches happen. |
...
| `dn` verifies the tree is clean | `dn/SKILL.md:50-54` | 224 | One of two places `rn` re-reads another rule's output rather than asserting its own — `task-verify-workflow.md:219-222`'s backstop is the other (row 71). It re-runs `git status --porcelain` against step 6's effect and records whatever step 6 failed to resolve. | Yes, and it costs one command. Nothing checks that *this* step ran (row 224), which is why the eight rows resting on it sit in U10. |
...
| `up` **4. Sync tasks** | `:26` | 36, 67, 68, 232 | The only reconciliation between what `git log` says was finished and what `steering.md` says is unchecked — and `rn`'s only consumer of the `complete task #{id}` marker, which is what makes seven rows elsewhere in this inventory enforced at all. | **Yes in principle, defective as built, and it runs on every resume.** The match is a substring test: `complete task #1` also matches `#1b` and `#10`–`#16` (`git log --all --grep='complete task #1' --oneline` returns 16 here, 7 of them other tasks), and one commit carries a real task's substring in prose without being a marker (`de0b1ec`). Every wrong match checks off a task nobody finished. |
...
| `ty` **2. Identify the pending approval** | `ty/SKILL.md:15` | 240 | The only step in `rn` that supplies its own reader: it states the target back to the user before anything is recorded, so a misidentified approval reaches the one party who knows what they meant. | Yes, always, and it is a sentence. |
...
| **Never resolve the thread** — resolution is the author's act | `:124`, `:142-148` | 203, 206 | Row 206's self-repair, and the reason rows 190, 191, 197–201 and 248 are enforced at all: the queue is GitHub's unresolved flag, not the assistant's memory. | Yes, unconditionally, and it costs nothing to obey. |
