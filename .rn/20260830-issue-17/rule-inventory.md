...
| 93 | "**Design sign-off task.** When the session has a `design.md` not settled at plan time, place a **"Design sign-off"** task … at the point where heavy build would otherwise start" | :36 | unseen | No rule checks that the task exists. The Evaluation sign-off's backstop (row 71) has no counterpart here — though that backstop is itself `unseen`, so the asymmetry buys less than it looks. **The breach count is 0 of 8, not 7 of 8**: `planning-workflow.md:36` places the task only when the session has a `design.md` *not settled at plan time*, and only 3 of the 8 sessions have a `Design:` line at all (`grep -n '^Design:' .rn/*/steering.md` returns `20260625-rn-lean`, `20260705-improve-design-template` and this one). Of the two past sessions that owed the question, both recorded the fold-in: `.rn/20260705-improve-design-template/steering.md:68-69` ("no separate Design sign-off task: the design was settled through discussion before this steering was written, so its approval folds into the plan gate") and `.rn/20260625-rn-lean/steering.md:78` ("a session with no design has none and the design gate folds into the plan gate"). Row 102 records the same branch. What is unenforced is the *judgment*, not a demonstrated omission. |
