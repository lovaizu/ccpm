...
- **enforced** — a breach reaches something that behaves differently because of it. This is the union
  of the `refuses` and `recognised` verdicts, and it is what "the enforced set" below means. The mechanism
...
By verdict: **13 refuse**, **80 are recognised on breach**, **161 are unseen**. Every one of the 13
refusals rests on something outside `rn`'s prose — a `git` or `gh` command that fails (72, 98, 99,
187, 188, 189, 192, 223) or the harness refusing to let the model invoke a skill
(`disable-model-invocation: true` — 208, 212, 226, 237, 244). **Not one rule in `rn` is
self-enforced.**
...
no validator, no CI step anywhere in the plugin. So **no rule in `rn` is self-enforced**. Only four
things can catch a breach, and every `refuses` and `recognised` row below rests on one of them:
...
3. **The user** — present at exactly three scheduled gates and on the PR. What the gate *presents*
   for their verdict is recognised; what merely appears on their screen is not, because they open a
   message to answer it, not to audit it. That distinction is what separates group C from U9.
...
| 44 | "Confirm the change matches the task's scope and Completion criteria before spending review experts." | :146 | unseen | A private judgement with no recorded output. |
...
| 52 | QA checklist — "the verification approach is meaningful to the actual objective … no rubber-stamped or purpose-mismatched check" | :162-163 | unseen | Nothing checks the reviewer against its checklist. |
...
| 62 | "Never silently drop, blindly accept, or bounce a finding for lack of a standard." | :191 | unseen | By construction: a silently dropped finding leaves no artifact. |
...
| 80 | Slug candidates: the current git branch, an issue reference in `$ARGUMENTS`, a kebab-case name from the goal | :15-17 | unseen | Nothing reads the slug back. |
...
| 83 | "**Check for an existing design.md first.** … This is a judgment call on scope overlap, not a mechanical file-existence check." | :21-26 | unseen | No artifact records that the check happened or what it concluded. |
...
| 91 | "Work backwards from the Acceptance criteria end state" | :35 | unseen | An unrecorded reasoning method; the task list looks the same either way. |
...
| # | Rule | Source | Verdict | Mechanism, or why nothing catches it |
|---|---|---|---|---|
| 105 | "Read when creating a new `steering.md`." | :3 | unseen | Nothing records the read. |
...
| 109 | Doc-division: "**Requirements & acceptance criteria → `steering.md`**" | :15 | unseen | No reader checks what kind of content sits in which file. |
...
| 112 | "A decision lands in a task, in `design.md`, or in a rule. Deliberation and history live in git + the PR — never in steering." | :19 | unseen | Nothing measures steering's content against this. |
...
| 117 | "two axes: goal alignment + quality" | :42 | unseen | Nothing checks that both axes are present. |
...
| 120 | "distinguish facts from assumptions — state explicitly if unverified" | :48 | unseen | Nothing separates the two after the fact. |
...
| 131 | "objectively verifiable by a third party; no vague terms ("appropriate", "correct")" | :78 | unseen | Nothing scans for the vague terms it names. |
...
| 143 | "**Emit only while a session is active** — its `steering.md` exists and is identified." with the listed exceptions (planning's pre-persist asks, `/rn:up` before identification, `/rn:gm` or `/rn:ty` with no active session) | :9-12 | unseen | Same reader as row 142. A block emitted with no session would have to be invented, which shows as fabricated task ids. Same as row 142. A block emitted with no session would carry invented task ids, which nothing compares to a file that does not exist. |
| 144 | "**Asks and flow-ending reports both count as stops** … on a report the 👉 line states where the session stands and the user's next move instead of an ask." | :13-16 | unseen | Same reader as row 142. Same as row 142. |
...
| 146 | "**A task counts ✅ when `steering.md` records it complete** — checked off, or carrying an explicit done annotation … Done-but-awaiting … still counts ✅; the pending item goes on the outlook line" | :19-22 | unseen | Row 141's paired rule. The user reads the block against the PR. Same as row 142 — and note the asymmetry: `steering.md`'s check-offs are a real input to this rule (rows 141, 216), but the block this rule produces has no reader but the user. |
| 147 | "**Write the block in the user's conversation language.**" | :23-24 | recognised | Immediately obvious to the user. |
| 148 | "**Markers are fixed**: ✅ completed / 👉 current / ⬜ remaining." | :25 | unseen | Same reader as row 142. Same as row 142. |
| 149 | The format block — header / ✅ / 👉 / ⬜ / outlook, in that order | :29-35 | unseen | Same reader as row 142. Same as row 142. |
| 150 | "**Header** — `── {slug}: {goal one-liner} ──`: the session's slug (the steering directory name, date prefix dropped) and a one-line compression of steering's `Goal`." | :37-38 | unseen | Same reader as row 142. Same as row 142. |
| 151 | "**✅ completed** … Group consecutive ids into ranges … comma-separate non-consecutive groups … No completed tasks yet → no ✅ lines." | :39-42 | unseen | Same reader as row 142. Same as row 142. |
| 152 | "**👉 current** — exactly one line … A stop not tied to a numbered task (the plan gate; an escalation that spans tasks) names the gate or moment instead of an id." | :43-48 | unseen | Same reader as row 142. Same as row 142. |
| 153 | "**⬜ remaining** … **No remaining tasks → omit the ⬜ lines entirely** — never render an empty ⬜ section." | :49-50 | unseen | Same reader as row 142. Same as row 142. |
| 154 | "**Outlook** — one closing parenthesized line: what follows this stop" | :51-52 | unseen | Same reader as row 142. Same as row 142. |
...
| 157 | "Reconcile the three artifacts below **in this order** — steering, then design, then tasks." | :15-18 | unseen | Order leaves no trace in the resulting commit. |
...
| 162 | For a task whose Completion criteria changed and that has a `checks/{task-id}.md`: "record in its Overall Verdict a `Ready to check off: No — criteria reconciled, self-check/review must re-run` line" — and if no such file exists, write nothing | :41-52 | unseen | **This row's own stated mechanism does not exist.** `:46-48` claims "`task-verify-workflow.md`'s Phase: Complete only checks off once Verify has cleared with `Ready to check off` reading Yes", but Phase: Complete (`task-verify-workflow.md:208-209`) never mentions the field: it says only "With Verify cleared … check off the task in `steering.md` directly." Empirically, 24 of 40 past check files carry no `Ready to check off` line at all and every one of those tasks was checked off anyway. |
...
| 167 | "The h2 sections below are the canonical design-document sections; under each, the h3 headings are the questions that section must answer — a section is done when its questions are answered, not when its heading merely exists." | :7-9 | unseen | Nothing distinguishes an answered h3 from a heading with prose under it. |
| 168 | "This is the fresh-authoring path … to update an existing one, skip to "Updating an existing design.md"" | :13-14 | unseen | Which path was taken is never recorded. |
...
| 171 | "**In Detailed design, repeat `4.N` once per mechanism or component the design introduces** — not a fixed four" | :23-24 | unseen | Nothing enumerates the design's mechanisms to compare against the subsection count. |
...
| 176 | "**Read the existing document in full before touching it.** Identify which h3 questions the session's work actually changes" | :104-105 | unseen | Reading leaves no trace. |
...
| 180 | "**Give genuinely open content a destination outside `design.md`.** … Session-scoped: put it in that session's `steering.md` Notes field … Canonical/cross-session: track it outside `design.md` instead (e.g. a repo issue)" | :127-137 | unseen | Nothing follows an open question to its destination; a dropped one leaves no residue. |
...
| 193 | "Process the queue **one thread at a time**. Never dispatch two threads in parallel" | :86-87 | unseen | Parallel dispatch produces the same replies in a different order; nothing records timing. |
...
| 205 | "When the queue is empty, the loop is done — report the loop result … opening that report with the session-status block" | :136-138 | unseen | Row 142's reader. Same as row 142. |
...
| 230 | "From step 3 on, any message stopping for user input opens with the session-status block" | `up/SKILL.md:22` | unseen | Row 142's reader. Same as row 142. |
...
| 242 | "**Advance the workflow.** … a plan or design gate passes — execution proceeds to the next task; … an evaluation gate passes — the session can close; … a reviewed item is accepted … When the advance ends the flow … open that closing report with the session-status block" | `ty/SKILL.md:19-24` | unseen | Row 142's reader. Same as row 142. |
| 243 | "**Nothing pending.** If nothing is actually awaiting approval, open the reply with the session-status block …, say so, and do nothing else." | `ty/SKILL.md:26` | unseen | Row 142's reader. Same as row 142. |
...
| Draft PR with a single-link body | `planning-workflow.md:44` | Puts the plan in front of the user in rendered form and creates the session's one review surface, on which mechanism C depends entirely. | Yes — every `recognised` verdict in this inventory that names the user ultimately resolves to this PR. |
