...
### D-5: 最終成果物は「計画」。提案書はその時点の計画から出すビュー、作業計画は計画の一部

計画 1 本が正。AI が各見出しを起案し、人は判断（選ぶ・承認する）だけ。提案書は提案時点の計画から一方向に
生成するビューで、出したあと更新しない。見積書は別に作らない。提案書の費用欄が見積の要約で、根拠となる
人日レベルの作業計画（作業項目・in/out・担当・確認者・Client 確認）は Client に出さない内部の計画として計画の中に持つ。計画はプロジェクト開始後も決定が変わるたびに直し続ける（ChangeLog に理由を残す）。
