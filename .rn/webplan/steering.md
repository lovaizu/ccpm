...
- 〔事実・確認済〕2026-09-18 に先方から要望資料（PDF）と注記付きモック HTML 3 枚（トップ・記事通常・記事連載）を
  受領。構成は先方が決定済で、注記の大半が決定、一部が「検討」＝こちらが判断して返す。色・書体・余白は
  たたき台と明記されており、要件ではない。
- 〔事実・確認済〕実装は先方が自社で行う。先方が自社で実装する構成を自ら提示したため、
...
- **Notes**:
  - ブランチ `hposal-plugin`、PR #8（OPEN）。先方入力は
    `project/sample-project/input/client-request_20260918/`（PDF＋注記付きモック 3 枚。gitignore 下）。
