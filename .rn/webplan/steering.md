...
- 〔事実・確認済〕2026-09-18 に先方から要望資料（PDF 5 頁）と注記付きモック HTML 3 枚（トップ・記事通常・記事連載）を
  受領。構成は先方が決定済で、注記 63 件のうち 57 件が決定、6 件が「検討」＝こちらが判断して返す。色・書体・余白は
  「たたき台であり、デザイナー様のご提案を優先します」と明記されており、要件ではない。
- 〔事実・確認済〕実装は先方が自社で行う。先方が「コーダーを立てずに弊社で実装する」構成を自ら提示したため、
  Provider はデザインのみ。HTML/CSS も納めない。
- 〔事実・確認済〕Figma の Dev Mode は Starter（無料）では使えず、課金はファイルを所有するチーム側にかかる。
  Variable Modes も Starter は 1 つだけで、ダーク／ライトの切替は無料では持てない。
- 〔判断〕提案の軸を「Provider がどこまで実装を担うか（A/B/C）」から「デザインの範囲をどこまで広げるか」に
  組み替える。B・C 案は「ご希望があれば」の注記に降格する。
- 〔判断〕構成は先方のモックのまま受け、問い直さない。提案の中身は見た目（配色・書体・余白・質感）と、
  先方が「ご提案ください」「ご定義ください」と書いた未決 10 項目への回答に置く。
- 〔判断〕telldes を紹介として提案に含める。Figma ファイル単体より使えるため。開発中である旨は明示する。
- 〔判断〕納期は日付を切らず「開始からこれくらいで順に完成していく」形で示す。
...
- [ ] モックの全要素を参考サイト 3 つと突き合わせ、〔決定／たたき台／要提案〕に切り分けて `work/mock-review.md` に置く
...
- **Notes**:
  - ブランチ `hposal-plugin`、PR #8（OPEN）。先方入力は
    `project/sample-project/input/client-request_20260918/`（PDF 5 頁＋注記付きモック 3 枚）。
  - `mock-review.md` の作り方：モックの注記 63 件＋注記の無い要素（レイアウト・ブレークポイント・
