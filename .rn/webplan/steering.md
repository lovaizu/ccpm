...
- 〔事実・確認済〕先方から要望資料と注記付きモックを受領。構成は先方が決定済で、注記の大半が決定、一部が
  こちらの判断を求める事項。色・書体・余白はたたき台と明記されており、要件ではない。
- 〔事実・確認済〕実装は先方が自社で行う構成を先方が自ら提示したため、Provider はデザインのみ。HTML/CSS も納めない。
