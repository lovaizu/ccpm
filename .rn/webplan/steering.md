...
契約形態…）を一つずつ人と決め、決定が揃ったら提案書＋見積に畳む」こと。提案書と見積はその出力であって目的ではない。
...
利用者＝Web サイト制作の提案者（Provider）。場面＝案件の相談を受けてから提案書と見積を出すまで。基準は利用者が
...
1. **何を決めればよいか迷わない** — 決めるべき事項が最初から全部見えていて、順に決めていけば漏れなく揃う。
   - 確認：#1 とは別の案件で最後まで進め、途中で「決め忘れ」に気づいて戻ることが起きない。手順の外で人が補った
     決定が無い。
2. **決定が揃えば提案書と見積が出る** — 決めた内容以外を書く作業が無く、提案書・見積・決定の間に食い違いが無い。
...
工程とロール）で提案書と見積まで仕上げる。作業のたびに `project/worklog.md` へ〔決めたこと／順序／入力／成果物／
...
- 提案者として、仮案件で「決めごとを決める → 提案書＋見積に畳む」を自分の手で最後まで経験し、提案書と見積が施主に
...
- `ccpm` から install した状態で提案者がスキルを起動し、設計どおりの順で決めごとを進めて提案書と見積に到達できる
  （1 案件で最後まで通る）。
...
- **Status**: paused
- **Date**: 2026-09-08
- **Last completed**: #1 Step 3（計画 `project/sample-project/plan.md` を全見出し起案し、ユーザーの通しレビューで約 20 件反映。learnings 15〜36 を push 済み）。
...
- **Notes**:
  - PR #8（OPEN）、ブランチ `hposal-plugin`。案件フォルダ `project/` は gitignore。数値の正は `project/sample-project/work/plan.data.json`、`work/render.py plan.md out work/plan.data.json` で提案書・見積を生成し額の一致を検査。
