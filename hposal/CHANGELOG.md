...
- The proposal chapter guide now maps each chapter to the actual 17-page deck and flags the sections the base slide template omits (proposer track record & team, post-launch care, contact), with instructions to add them — so a required "who we are" section isn't dropped just because the skeleton lacks a slot.
