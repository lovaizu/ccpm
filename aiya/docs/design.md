...
The design of aiya: what it is, and the decisions behind it with their intent. Each section answers
one question; top to bottom they run: why build this → what is given → the core idea → the pieces and
the flow → each part exactly → why this shape.

## 1. Why build this, and what must hold?
...
### 1.2 What does reaching it require?
...
### 1.3 What does an AI-agent attempt run into?

Trying to build that with AI agents surfaces two
...
- **Parallel dispatch.** "Many work-streams" means many units of work under one goal, not simultaneous
  execution — Turns run sequentially, 1 Step = 1 Turn (§4.1). Concurrency is out of scope at this stage.
...
## 2. What is given, and what binds us?
...
- **No controller program.** Self-imposed (§6, first entry): the loop is carried by procedure prose.
...
- **Existing surfaces only.** Human touchpoints ride existing async chat (§5.4); no dedicated UI is
  built.
...
**bounded state handed between Turns** (the CCS, §5.1), and a **traceability chain checked at gates**
(§5.4). The result is that the bounded, on-target path is the **default** path, and adherence is
**observable**.

## 4. What are the pieces, and how does work move?

### 4.1 What are the pieces?

**The cast, at a glance** — four actors, and the two boundaries that make the design work.
...
| **CCS** | The bounded state handed Turn-to-Turn — the only bridge the Conductor reads (§5.1). |
| **phase gate** | A boundary where the human approves or redirects (§5.4). |
...
(Stage ③, §4.2), adding further work-Turns within that same Step, capped at 3 attempts total (§5.3).
...
## 5. How does each part behave, exactly?

One section per load-bearing contract. Coverage against §4.1's parts table: the CCS and the verify-Turn
have their own sections (§5.1, §5.2); the Conductor's contract is §5.1 rules 3–4 plus §5.3; the
generate-Turn's is Stage ① (§4.2) plus §5.1 rules 1–2; the phase gates are §5.4.
...
boundedness (§5.1 rule 3) still holds.
...
- **The bounded path is the default, and adherence is observable** (§3), through three mechanisms of
  ...
  the push channel regardless of Turn behavior. The Conductor's **read-restriction** (§5.1 rule 4) is a
  ...
  grep invariant** (§5.1 rules 1–2) is a bounding convention: exceeding the soft cap is a visible health
