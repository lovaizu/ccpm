...
**Catching breaches.** Rule 1 is structural — fresh contexts come from the platform. Rule 2 is structural for the spawn tool — it is absent from every Turn definition, and a tool left out of a definition is not in the Turn's session at all (the user-question tool is left out the same way — Turns never talk to people); for the definitions that carry Bash, which could in principle start a nested platform session, the rule is default and observable (§2), not absolute. Rule 4 the Conductor checks on every return: it is bounded, or it is not.
