...
2. **Enter at the pending gate** (conductor §3, §8). Record the approval — an Output Gate approval fixes the document's current version, written by Turn(generate), as the approved yardstick (G1/G2/G3) — and advance: a Planning Gate opens the phase's dispatch round; G1 and G2 fix the next phase's yardstick; G3 completes the run. If nothing is pending, say so and do nothing else.
