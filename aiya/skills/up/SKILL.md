...
2. **Enter at resume** (conductor §9). Locate the run by run directory — the `.aiya/` directory carrying a `backend` file; if several, the most recently modified, and say so in the announcement; if none exists, say so and point at `/aiya:on`. Reread its `backend` file so record dispatches go to the right adapter. Read exactly what the next wave would have read — the latest CCS, the approved phase documents, the current Plan — announce the position (e.g. "Resuming csv-export — Wave 2, #4, attempt 2"), and continue the dispatch round from there. When `ccs/` is empty, no wave has run yet: the run stands at its first pending gate — read the position from the Plan and `verdicts/` instead, and name that gate in the announcement. When what is on disk cannot distinguish a gate still awaiting its verdict from one just approved — a Planning Gate's approval writes nothing new — re-present the pending gate and wait: the safe side, costing at most one redundant `/aiya:ty`.
