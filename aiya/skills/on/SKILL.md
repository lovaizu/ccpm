...
2. **Enter at the top.** Create the run directory (conductor §2) from a short slug of `$ARGUMENTS`, detect the platform backend and record it to the run's `backend` file, and dispatch the matching Turn(record) to open the review surface — the run branch, first push, and PR/MR under github or gitlab; under local the dispatch still happens — the adapter confirms the run directory and reports it as the surface (conductor §2). The surface exists before the first gate points at it. Then begin the Purpose phase: draft Plan(Purpose) (conductor §4), have it verified against the fixed viewpoints, and stop at the Purpose Planning Gate — a bounded console summary of how the purpose will be pinned down, answered with `/aiya:ty` or `/aiya:gm`.
